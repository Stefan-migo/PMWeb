...
test("authorized media upload persists a delivery URL and rejects invalid files", async ({ page }) => {
  ...
  await expect(page.locator("[data-artwork-id]").filter({ hasText: slug })).toBeVisible({ timeout: 30000 });
  ...
  await expect(page.locator("[data-artwork-id]").filter({ hasText: slug })).toHaveCount(0, { timeout: 30000 });
});
